        ...
        <div class="container mx-auto py-8">
            <div class="flex flex-col md:flex-row gap-6">
                <div class="w-full md:w-1/3 space-y-8">
                    <div class="bg-white shadow rounded-lg p-6">
                        <div class="m-10 max-w-sm">
                                ...
                                <span
                                    class="font-medium text-white text-3xl">{{ getInitials(type.name) }}</span>
                                    </div>
                                    <h1 class="py-2   text-xl font-bold leading-8 text-gray-900">{{uppercase(type.name)}}</h1>
                                </div>
                ...
                <div class="w-full md:w-2/3 space-y-8">
                    <div class="bg-white shadow rounded-lg p-6">
                        <h2 class="text-xl font-bold mb-4">Collateral type Information</h2>
                        <div class="flex justify-between flex-wrap gap-2 w-full mb-6">
                            <span class="text-primary font-bold">Type Created At</span>
                            <p>
                                <span class="text-gray-700 mr-2">{{formatDate(type.created_at)}}</span>
                            </p>
                        </div>
                        <div class="flex justify-between flex-wrap gap-2 w-full mb-6">
                            <span class="text-primary font-bold">Created By</span>
                            <p>
                                <span class="text-gray-700 mr-2">{{type.user.name}}</span>
                            </p>
                        </div>
                        <div class="flex justify-between flex-wrap gap-2 w-full mb-6">
                            <span class="text-primary font-bold">Description</span>
                            <p>
                                <span class="text-gray-700 mr-2">{{type.description}}</span>
                            </p>
                        </div>
