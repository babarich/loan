<template>
    <AppLayout>
        <div class="flex justify-between mb-6 mt-4">
            <div class="mt-2">
                <h3 class="text-xl text-primary text-semibold">Due schedules</h3>
            </div>
            <div>
        ...
        <div class="rounded bg-white p-5 shadow md:p-8 w-full">
                ...
                <table class="w-full text-sm whitespace-nowrap">
                    ...
                    <tr>
                        ...
                        <TableHeaderCell field="reference" :sort-field="sortField" :sort-direction="sortDirection"
                                         @click="sortTransaction('reference')"> Borrower</TableHeaderCell>

                        <TableHeaderCell field="reference" :sort-field="sortField" :sort-direction="sortDirection"
                                         @click="sortTransaction('name')">schedule</TableHeaderCell>
                    ...
                    <tr v-for="(schedule,index) in schedules.data" :key="index" class="odd:bg-gray-100 focus-within:bg-gray-100">
                        <td class="py-4 px-4 text-sm text-left border">
                            {{index + 1}}
                        </td>
                        <td class="py-4 px-4 text-sm text-left border">{{schedule.name}}</td>
                        <td class="py-4 px-4 text-sm text-left border">{{schedule.product}}</td>
                        <td class="py-4 px-4 text-sm text-left border">{{schedule.principal}}</td>
                        <td class="py-4 px-4 text-sm text-left border text-wrap">{{schedule.interest}}</td>
                        <td class="py-4 px-4 text-sm text-left border text-wrap">{{schedule.due}}</td>
                        <td class="py-4 px-4 text-sm text-left border">{{schedule.paid}}</td>
                        <td class="py-4 px-4 text-sm text-left border">
                        <span class="bg-yellow-500 text-white p-2 rounded-md text-xs" v-if="schedule.status === 'pending'">
                            Pending
                        </span>
            ...
            <pagination class="mt-6" :data="schedules" :links="schedules.links" @getPage="getChanges"/>
        </div>
    </AppLayout>
</template>
...
const props = defineProps({
    schedules:Object, errors:Object
});
