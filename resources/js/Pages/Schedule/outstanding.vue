<template>
    <AppLayout>
        <div class="flex justify-between mb-6 mt-4">
            <div class="mt-2">
                <h3 class="text-xl text-primary text-semibold">Principal Outstanding</h3>
            </div>
            <div>
        ...
        <div class="rounded bg-white p-5 shadow md:p-8 w-full">
            ...
            <div class="bg-white overflow-x-auto">
                <table class="w-full text-sm whitespace-nowrap">
                    ...
                    <tr>
                        ...
                        <TableHeaderCell field="reference" :sort-field="sortField" :sort-direction="sortDirection"
                                         @click="sortTransaction('name')">Release Date</TableHeaderCell>

                        <TableHeaderCell field="reference" :sort-field="sortField" :sort-direction="sortDirection"
                                         @click="sortTransaction('name')">Maturity Date</TableHeaderCell>
                        ...
                        <TableHeaderCell field="principal" :sort-field="sortField" :sort-direction="sortDirection"
                                         @click="sortTransaction('interest')"> Principal paid</TableHeaderCell>

                        <TableHeaderCell field="principal" :sort-field="sortField" :sort-direction="sortDirection"
                                         @click="sortTransaction('interest')">  Principal Balance</TableHeaderCell>
                    ...
                    <tr v-for="(out,index) in outs.data" :key="index" class="odd:bg-gray-100 focus-within:bg-gray-100">
                        <td class="py-4 px-4 text-sm text-left border">
                            {{index + 1}}
                        </td>
                        <td class="py-4 px-4 text-sm text-left border">{{out.reference}}</td>
                        <td class="py-4 px-4 text-sm text-left border">{{out.name}}</td>
                        <td class="py-4 px-4 text-sm text-left border">{{out.release}}</td>
                        <td class="py-4 px-4 text-sm text-left border">{{out.maturity}}</td>
                        <td class="py-4 px-4 text-sm text-left border">{{out.principal}}</td>
                        <td class="py-4 px-4 text-sm text-left border">{{out.paid}}</td>
                        <td class="py-4 px-4 text-sm text-left border text-wrap">{{out.balance}}</td>
                        <td class="py-4 px-4 text-sm text-left border">
                        <span class="bg-yellow-500 text-white p-2 rounded-md text-xs" v-if="out.status === 'pending'">
                            Pending
                        </span>
            ...
            <pagination class="mt-6" :data="outs" :links="outs.links" @getPage="getChanges"/>
        </div>
    </AppLayout>
</template>
