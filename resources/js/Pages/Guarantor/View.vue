...
<AppLayout>
    <div class="mt-4 mb-3 flex flex-col md:flex-row items-center justify-between">
        ...
        <div>
            <a :href="route('guarantor.index')" class="px-4 text-sm flex text-gray-100 bg-primary p-2 rounded">
    ...
    <div class="bg-gray-100">
        <div class="container mx-auto py-8">
                    ...
                    <div class="bg-white shadow rounded-lg p-6">
                        <div class="m-10 max-w-sm">
                                <div class="relative mx-auto w-52 rounded-full">
                                    <img :src="guarantor.filename" class="object-cover h-52 bg-gray-300 rounded-md mb-4 shrink-0" :alt="guarantor.first_name" />
                                </div>
                                <h1 class="my-1 text-center text-xl font-bold leading-8 text-gray-900">{{uppercase(guarantor.title)}}. {{uppercase(guarantor.first_name)}} {{uppercase(guarantor.last_name)}}</h1>
                                <h3 class="font-lg text-semibold text-center leading-6 text-gray-600">{{guarantor.business_name}}, {{guarantor.working_status}}</h3>
                                <ul class="mt-3 divide-y rounded bg-gray-100 py-2 px-3 text-gray-600 shadow-sm hover:text-gray-700 hover:shadow">
                                    <li class="flex flex-col md:flex-row items-center py-3 text-sm">
                                        ...
                                        <span class="ml-auto">{{formatDate(guarantor.date_birth)}}</span>
                                    </li>
                                    <li class="flex flex-col md:flex-row items-center py-3 text-sm">
                                        <span class="text-primary font-semibold">Gender</span>
                                        <span class="ml-auto">{{uppercase(guarantor.gender)}}</span>
                                    </li>
                                    <li class="flex flex-col md:flex-row items-center py-3 text-sm">
                                        <span class="text-primary font-semibold">Joined On</span>
                                        <span class="ml-auto">{{formatDate(guarantor.created_at)}}</span>
                                    </li>
                                    <li class="flex flex-col md:flex-row items-center py-3 text-sm">
                                        <span class="text-primary font-semibold">Loan Officer</span>
                                        <span class="ml-auto">{{guarantor.user.name}}</span>
                                    </li>
                                </ul>
                ...
                <div class="w-full md:w-2/3 space-y-8">
                    <div class="bg-white shadow rounded-lg p-6">
                        ...
                        <div class="flex justify-between flex-wrap gap-2 w-full mb-6">
                            ...
                            <p>
                                <span class="text-gray-700 mr-2">{{guarantor.address}}</span>
                            </p>
                        </div>
                        <div class="flex justify-between flex-wrap gap-2 w-full mb-6">
                            <span class="text-primary font-bold">City</span>
                            <p>
                                <span class="text-gray-700 mr-2">{{guarantor.city}}</span>
                            </p>
                        </div>
                        <div class="flex justify-between flex-wrap gap-2 w-full mb-6">
                            <span class="text-primary font-bold">Mobile</span>
                            <p>
                                <span class="text-gray-700 mr-2">{{guarantor.mobile}}</span>
                            </p>
                        </div>
                        <div class="flex justify-between flex-wrap gap-2 w-full mb-6">
                            <span class="text-primary font-bold">Email</span>
                            <p>
                                <span class="text-gray-700 mr-2">{{guarantor.email}}</span>
                            </p>
                        </div>
                        <div class="flex justify-between flex-wrap gap-2 w-full mb-6">
                            <span class="text-primary font-bold">Description</span>
                            <p>
                                <span class="text-gray-700 mr-2">{{guarantor.description}}</span>
                            </p>
                        </div>
                        <h2 class="text-xl font-bold mb-4">Guarantor Files</h2>
                        <div class="bg-white relative overflow-x-auto mt-4">
                            <table class="w-full text-sm whitespace-nowrap">
                                ...
                                <tr class="odd:bg-gray-100 focus-within:bg-gray-100"
                                    v-for="(attach, index) in guarantor.attachments" :key="index">
                                    ...
                                    <td class="py-4 px-4 text-sm text-left border">
                                        <a :href="route('guarantor.download', attach.id)" class="px-4 text-sm flex text-gray-100 bg-primary p-2 rounded">
            ...
            <div class="rounded bg-white p-5 shadow md:p-8 w-full mt-4">
                <h3 class="text-primary text-2xl font-semibold mb-6">Borrowers Information</h3>
                <div class="mt-4 mb-3 flex flex-col md:flex-row items-center justify-between">
