<template>
    <AppLayout>
        <div class="flex justify-between mb-6 mt-4">
            <div class="mt-2">
                <h3 class="text-xl text-primary text-semibold">Guarantors</h3>
            </div>
            <div>
                <NButton :href="route('guarantor.create')" class="text-white">
                    <PlusCircleIcon class="w-4 h-4  text-white mr-2"/>  Add Guarantor
                </NButton>
        ...
        <div class="bg-white overflow-x-auto">
            <table class="w-full text-sm whitespace-nowrap">
                ...
                <tbody>
                <tr v-for="(guarantor,index) in guarantors.data" :key="index" class="odd:bg-gray-100 focus-within:bg-gray-100">
                    <td class="py-4 px-4 text-sm text-left border">
                        {{index + 1}}
                    </td>
                    <td class="py-4 px-4 text-sm text-left border">{{guarantor.name}}</td>
                    <td class="py-4 px-4 text-sm text-left border">{{guarantor.reference}}</td>
                    <td class="py-4 px-4 text-sm text-left border">{{guarantor.email}}</td>
                    <td class="py-4 px-4 text-sm text-left border">{{guarantor.mobile}}</td>
                    <td class="py-4 px-4 text-sm text-left border text-wrap">{{guarantor.business}}</td>
                    <td class="py-4 px-4 text-sm text-left border">
                        <span class="bg-yellow-500 text-white p-2 rounded-md text-xs" v-if="guarantor.status === 'pending'">
                            Pending
                        </span>
                    ...
                    <td class="py-4 px-4 text-sm text-left border">
                     <div class="flex justify-between flex-col md:flex-row items-center">
                         <div class="flex items-center space-x-2 divide-x">
                             <a :href="route('guarantor.show', guarantor.id)" class="px-4 text-sm flex text-gray-100 bg-emerald-600 p-2 rounded">
                                 <i class="pi pi-eye text-white"></i>
                             </a>
                             <a :href="route('guarantor.edit', guarantor.id)" class="px-4 text-sm flex text-gray-100 bg-primary p-2 rounded">
                                 <i class="pi pi-pencil text-white"></i>
                             </a>

                             <button @click.prevent="confirmDelete(guarantor.id)"  class="px-4 text-sm flex text-gray-100 bg-red-500 p-2 rounded">
            ...
            <pagination class="mt-6" :data="guarantors" :links="guarantors.links" @getPage="getChanges"/>
    </div>
    </AppLayout>
</template>
...
const props = defineProps({
   guarantors:Object, errors:Object
});
