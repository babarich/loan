<template>
    <AppLayout>
        ...
        <div class="h-full p-5 md:p-8">
            <div class="flex flex-col md:flex-row items-center justify-between  pb-5 md:pb-7">
                    <div>
                        <h1 class="text-lg font-semibold text-heading">Edit  Guarantor </h1>
                    </div>
                    <div>
            ...
            <Stepper v-model:activeStep="active">
                    ...
                    <template #content="{ nextCallback }">
                        <div class="flex flex-wrap pb-8 my-5 border-b border-dashed border-border-base sm:my-8">
                            <div class="w-full px-0 pb-5 sm:w-4/12 sm:py-8 sm:pe-4 md:w-1/3 md:pe-5">
                                ...
                                <p class="text-sm text-body">Add some basic info about your guarantor</p>
                            </div>
                ...
                <StepperPanel>
                    <template #header="{ index, clickCallback }">
                        <button class="bg-transparent border-none inline-flex flex-column gap-2" @click="clickCallback">
                          <span :class="['rounded-full p-2 flex item-center ' +
                         'justify-content-center', { 'bg-primary border-primary': index <= active, 'surface-border': index > active }]">
                            <i class="pi pi-bookmark text-white text-center" />
                        </span>
                            Address Information
                        </button>
                    </template>
                    <template #content="{ prevCallback, nextCallback }">
                        <div class="flex flex-wrap pb-8 my-5 border-b border-dashed border-border-base sm:my-8">
                            <div class="w-full px-0 pb-5 sm:w-4/12 sm:py-8 sm:pe-4 md:w-1/3 md:pe-5">
                                <h4 class="text-base font-semibold text-body-dark mb-2">Basic Info</h4>
                                <p class="text-sm text-body">Add some address and work information</p>
                            </div>
                            <div class="p-5  md:p-8 w-full sm:w-8/12 md:w-2/3">
                                <div class="mb-5"><label class="flex text-body-dark font-semibold text-sm leading-none mb-3"
                ...
                <StepperPanel>
                    ...
                    <template #content="{ prevCallback }">
                        <div class="flex flex-wrap pb-8 my-5 border-b border-dashed border-border-base sm:my-8">
                            <div class="w-full px-0 pb-5 sm:w-4/12 sm:py-8 sm:pe-4 md:w-1/3 md:pe-5">
                                <h4 class="text-base font-semibold text-body-dark mb-2">Basic Info</h4>
                                <p class="text-sm text-body">Add info about your files</p>
                            </div>

                            <div class="p-5  md:p-8 w-full sm:w-8/12 md:w-2/3">
                                <div>
                                    <label class="flex text-body-dark font-semibold text-sm leading-none mb-3"
                                           for="description">Upload Guarantors Photo</label>
                ...
                <svg
                  ...
                  />
                                ...
                                <label class="flex text-body-dark font-semibold text-sm leading-none mb-3"
                                       for="description">Add Guarantors additional Files</label>
...
const props = defineProps({
    errors:Object,guarantor:Object
});

const form = useForm({
    firstname: props.guarantor.first_name,
    lastname: props.guarantor.last_name,
    file: null,
    birth: formatDate(props.guarantor.date_birth),
    address:props.guarantor.address,
    gender:props.guarantor.gender,
    city:props.guarantor.city,
    description:props.guarantor.description,
    title:props.guarantor.title,
    mobile:props.guarantor.mobile,
    email:props.guarantor.email,
    working:props.guarantor.working_status,
    business:props.guarantor.business_name,
    image: props.guarantor.filename,
    image_url: props.guarantor.filename,
});
...
function onUpdate(){
    loading.value = true
    form.post(route('guarantor.update', props.guarantor.id),{
        onSuccess:()=>{
            loading.value = false
        },
        onError:()=>{
