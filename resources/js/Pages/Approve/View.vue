    ...
    <AppLayout>
        <div class="mt-4 mb-3 flex flex-col md:flex-row items-center justify-between">
            <div class="flex justify-between">
                ...
                <a   class="px-4 text-sm flex text-gray-100 bg-emerald-600 p-2 rounded mr-4 cursor-pointer"
                     :href="route('approve.approveFirst', loan.id)" v-if="can().loan_approve">
                    <i class="pi pi-check w-4 h-4 mr-2"></i>  Approve
                </a>

                <a @click="returnFirst = true" class="px-4 text-sm flex text-gray-100 bg-yellow-500 p-2 rounded mr-4 cursor-pointer"
                v-if="can().loan_return">
                    <i class="pi pi-spinner w-4 h-4 mr-2"></i>  Return
                </a>

                <a :href="route('approve.rejectFirst', loan.id)" class="px-4 text-sm flex text-gray-100 bg-red-500 p-2 rounded cursor-pointer"
            ...
            <div class="flex justify-between">
                ...
                <a :href="route('approve.index')" class="px-4 text-sm flex text-gray-100 bg-primary p-2 rounded">
                    <ArrowLeftIcon class="w-4 h-4 mr-2"/>  Back
                </a>
            </div>
        </div>
        <Dialog v-model:visible="returnFirst" modal header="Add Return Comment" style="width:50%">
                    ...
                    <div class="col-span-12 lg:col-span-8 2xl:col-span-9">
                        <div class="bg-white shadow rounded-lg p-6 divide-y ">
                            ...
                            <div class="flex justify-between flex-wrap gap-2 w-full mb-6">
                                ...
                                <span class="bg-emerald-500 text-white p-2 rounded-md text-xs mt-2" v-if="loan.status === 'second disburser approved'">
                            Approved
                        </span>
                                <span class="bg-yellow-500 text-white p-2 rounded-md text-xs mt-2" v-else>
                            {{loan.status}}
                        </span>
...
function submitComment(){
    loading.value = true
    commentForm.post(route('approve.comment', props.loan.id),{
        onSuccess:()=>{
            loading.value = false
            returnFirst.value = false
            commentForm.reset()
        },
        onError:()=>{
            loading.value = false
